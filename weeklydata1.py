...
output_file = 'ytr_full_data.csv'
temp_folder = 'temp_ytr_data'  # Temporary folder for individual symbol data

# Create the temp folder if it doesn't exist
if not os.path.exists(temp_folder):
    os.makedirs(temp_folder)

# Function to fetch weekly data for a single stock
def fetch_weekly_data(symbol):
    print(f"Fetching data for {symbol} from maximum available history...")
    try:
        data = yf.download(symbol, interval='1wk')
...
if failed_symbols:
    with open('failed_symbols.txt', 'w') as f:
        f.write('\n'.join(failed_symbols))
    print(f"Failed symbols logged to 'failed_symbols.txt'.")
